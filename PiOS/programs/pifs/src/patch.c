...
//    cmd_append(console, "/console", &c, 1);
    ...
    if(!console) {
        int retv;
        console = file_lookup(&retv, root, "/console", perm_rd);  // Privileged -- write with read permission.
        if(!console) {
            fprintf(stderr, "<<< Could not find console...\n");
            return retv;
        }
    }
    if(console->f->n_alloc <= (console->f->n_data + 1)) {
        file_realloc(console->f, console->f->n_data + 1);
    }

    console->f->data[console->f->n_data] = c;
    console->f->n_data++;
    return 0;
}
...
int main(int argc, char *argv[]) {
    ...
    assert((reboot = ent_lookup(root, "/reboot.cmd")));
