...
void aux_uart_enable_uart(uint8_t mode) {
    uint32_t current = get32(&uart->cntl); // Preserve current settings
...
uint8_t aux_uart_receive() {
    return (uint8_t) get32(&uart->io); // BCM2835AP §2.2.2 p. 11 - When reading from this register, data is retrieved from the RX FIFO.
}
